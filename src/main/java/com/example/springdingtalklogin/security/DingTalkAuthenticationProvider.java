...
public class DingTalkAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        SysUser sysUser = userService.loginWithDingTalk((String)authentication.getCredentials());
        //此处可抛出用户相关异常，如账号禁用等
        if (sysUser == null) {
            throw new UsernameNotFoundException("用户不存在");
        }
        return new DingTalkAuthenticationToken(sysUser, this.createAuthorityList(sysUser.getPermissions().toArray(new String[]{})));
    }
